...
import com.kinstalk.her.incallui.ext.IInCallScreenExt;
    ...
    private boolean updateCallInMap(Call call) {
        ...
        if (call.getState() == Call.State.DISCONNECTED) {
            ...
            if (mCallById.containsKey(call.getId())) {
                ...
                /// M:add for plug in. @{
                mCallMap.put(call.getId(), call.getTelecomCall());
                /// @}
...
//            }
    ...
    *@return call id list.
    */
    private List<String> getIncomingCallIdList() {
        final List<String> callIdList = new ArrayList<String>();
        /*for (Call call: mIncomingCallList) {
            callIdList.add(call.getTelecomCall().getCallId());
        }*/
        return callIdList;
    }
    ...
    */
    ...
    /**
     * M: Get calls that is waiting for {@link PhoneAccount} selection.
     */
    public List<Call> getWaitingForAccountCalls() {
        List<Call> calls = new ArrayList<Call>();
        for (Call call : mCallById.values()) {
            if (call.getState() == Call.State.SELECT_PHONE_ACCOUNT) {
                calls.add(call);
            }
        }
        return calls;
