...
package com.kinstalk.her.incallui.widget;
...
public class FloatingActionButtonController {
    ...
    /**
     * 设置按钮是变灰还是可用
     * type =1  蓝色是dialtactsActivity的浮动按钮
     * type =2  绿色是DiapadFragment里面的打电话按钮
     */
    public void setEnabled(Context context, boolean enabled, int type){
        if(mFloatingActionButtonContainer.getBackground()!=null){
            if(type == 1) {
                mFloatingActionButtonContainer.getBackground().setTint(enabled ?
                        context.getColor(R.color.fab_blue_color) :
                        context.getColor(R.color.fab_gray_color));
            }else if(type == 2) {
                mFloatingActionButtonContainer.getBackground().setTint(enabled ?
                        context.getColor(R.color.fab_green_color) :
                        context.getColor(R.color.fab_gray_color));
            }
            mFloatingActionButton.setClickable(enabled);
        }
    }
