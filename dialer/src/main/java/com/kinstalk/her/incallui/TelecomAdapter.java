...
//            mInCallService.stopVoiceRecording();
    ...
    void setSortedIncomingCallList(List<String> incomingCallList) {
        if (mInCallService != null) {
//            mInCallService.setSortedIncomingCallList(incomingCallList);
        } else {
            Log.e(this, "error setSortedIncomingCallList, mInCallService is null");
        }
    }
...
//            mInCallService.explicitCallTransfer(callId);
    ...
    public void explicitCallTransfer(String callId, String number, int type) {
        if (mInCallService != null) {
//            mInCallService.explicitCallTransfer(callId, number, type);
        } else {
            Log.e(this, "error explicitCallTransfer, mInCallService is null");
        }
    }
...
//            mInCallService.hangupActiveAndAnswerWaiting();
    ...
    void sendMessageIfCallEnded(Context context, String callId, String phoneNumber,
                                String textMessage) {
        if (mInCallService != null && getTelecomCallById(callId) != null) {
            return;
        }
...
//        Toast.getInstance().show(confirmationMsg);
    ...
    public void inviteConferenceParticipants(String conferenceCallId, List<String> numbers) {
        if (mInCallService != null && getTelecomCallById(conferenceCallId) != null) {
//            getTelecomCallById(conferenceCallId).inviteConferenceParticipants(numbers);
        } else {
