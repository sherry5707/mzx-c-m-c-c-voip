...
//        }
    ...
    public void adjustPreviewLocation(boolean shiftUp, int offset) {
        ...
        /// M: shift up in Landscape is unnecessary @{
        if (mIsLandscape) {
            return;
        }
        /// M: @}
    ...
    /**
     * {@link Surface} on which incoming video for a video call is displayed.
     * {@code Null} until the video views {@link ViewStub} is inflated.
     */
    @Override
    ...
    /**
     * {@link Surface} on which a preview of the outgoing video for a video call is
     * displayed.  {@code Null} until the video views {@link ViewStub} is inflated.
     */
    @Override
...
//------------------------------------------Mediatek-------------------------------------
    ...
    public void resizeDisplaySurface() {
        ...
        /// M: Always re-set the surface rotation, due to the change of rotation wouldn't go with
        // the change of with or height. Note the method 'setRotation' would check if the rotation
        // had been changed really before it set the rotation to surface really. @{
        ...
        /// M: AOSP original, bring it to front.
//        displaySurface.setRotation(rotationAdjust);
...
//        }
     ...
     */
    @Override
    public void updatePeerDimensionAndAngle(int width, int height, int rotation) {
        mPeerVideoWidth = width;
        mPeerVideoHeight = height;
        mPeerVideoRotationAdjust = rotation;
    }
    ...
    public void onResume() {
        super.onResume();
//        VideoDebugInfo debugInfo = getPresenter().getDebugInfo();
//        if(debugInfo != null) {
//            logd("[checkPackageLoss]EngineerMode for package loss is on,set param");
//            debugInfo.setFragment(this);
//            if (!(mVideoViews instanceof FrameLayout)) {
//                logd("[checkPackageLoss]not a FrameLayout: " + mVideoViews);
//                return;
//            }
//            debugInfo.setFrameLayout((FrameLayout)mVideoViews);
//        }
    }
}
