                ...
                if (animator != null) {
                    animator.addListener(new AnimatorListenerAdapter() {
                        ...
                        public void onAnimationEnd(Animator animation) {
                            ...
                            Log.i(TAG, "[onPreDraw]animation end");
//                            InCallTrace.begin("CircularAnimation end");
                            view.setClipToOutline(false);
                            if (mListener != null) {
                                mListener.onCircularRevealComplete(getFragmentManager());
                            }
//                            InCallTrace.end("CircularAnimation end");
                            /// @}
                        }
                    });
                    Log.i(TAG, "[onPreDraw]animation triggered");
                    /// M: add for MO performance analysis @{
//                    InCallTrace.begin("CircularAnimation start");
                    animator.start();
//                    InCallTrace.end("CircularAnimation start");
                    /// @}
                }
                return false;
            }
    ...
    private Animator getRevealAnimator(Point touchPoint) {
        final Activity activity = getActivity();
        /// M: [ALPS02333975]Seldom, the getActivity() would return null. need error handling. @{
        if (activity == null) {
            Log.e(this, "Error handling for seldom case, no animation would be shown");
